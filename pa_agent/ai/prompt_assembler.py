...
A股/港股硬约束：必须输出顶层 `a_share` 对象；禁止输出开空、融券、杠杆或衍生品计划；看空只能写成风险提示、防守提醒或等待。只有 `a_share.action_type="long_plan"` 时才允许 `decision.order_direction="做多"` 并填写入场/止损/止盈；其他 action_type 一律 `order_type="不下单"`。
...
```json
{
  ...
  "a_share": {
    ...
    "constraints": ["不输出开空计划", "不输出融券/杠杆/衍生品计划", "不要求用户输入持仓", "不计算仓位数量"]
  },
  "diagnosis_summary": {
    "cycle_position": "",
    "direction": "",
...
- A股/港股只允许做多条件计划：风险点数 = entry − stop，回报点数 = take_profit − entry；不得输出做空公式、做空下单或开空价格
...
- A股/港股减仓/防守只能作为“已有持仓者”的风险提示；不要要求用户输入持仓、成本价或资金量，不计算仓位数量。
    ...
    def _build_stage2_user_prompt(
        ...
        *,
        ...
        return (
            ...
            f"A股/港股模式下必须同时输出 a_share；只给多头条件计划，看空表达为风险/防守/等待，禁止开空、融券、杠杆和衍生品计划。\n\n"
